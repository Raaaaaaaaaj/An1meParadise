...
import { isAdminAuthenticated, saveAdminSession } from "@/admin/routes/adminAuth";
...
const AdminLoginPage = () => {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const from = location.state?.from?.pathname?.startsWith("/admin") ? location.state.from.pathname : fallbackPath;
    ...
    <div className="admin-crm grid min-h-screen bg-background text-foreground lg:grid-cols-[1fr_28rem]">
      <section className="relative hidden border-r border-border bg-card lg:block">
        ...
        <div className="relative flex h-full flex-col justify-between p-10">
          <img src="/logo2.png" alt="AN1ME PARADISE" className="h-24 w-fit object-contain" />
          <div>
            ...
            <p className="mt-5 max-w-md text-sm leading-6 text-muted-foreground">
              Manage users, catalog, tags, orders, and contacts from one protected admin surface.
            </p>
          </div>
          <p className="text-xs text-muted-foreground">AN1ME Paradise Admin Panel</p>
        </div>
      </section>
      ...
      <main className="flex min-h-screen items-center justify-center px-4 py-10">
        ...
        >
          <div className="mb-8">
            ...
            <h2 className="mt-2 font-display text-2xl font-bold tracking-wider">Admin Login</h2>
          </div>
